...
3. SSH in (`ssh YOUR_USER@<pi-address>` — the account you created in Imager, not necessarily `pi`). Clone the public repository over HTTPS; no GitHub
   account, password, token, or personal SSH key is needed on the Pi.
